...
(async()=>{
 ...
 try{
  ...
  await page.getByRole('button',{name:'手牌を確認',exact:true}).click();
  const hands=page.getByRole('dialog',{name:'終局時の手牌'});await hands.waitFor();
  assert.equal(await hands.locator('.result-hand').count(),4);
  assert.equal(await hands.locator('.result-hand-tiles img[alt="伏せ牌"]').count(),0);
  assert.ok(await hands.locator('.result-hand-tiles img').count()>0);
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
  await page.screenshot({path:path.resolve(__dirname,'../test-results/result-hands.png'),fullPage:true});
  await hands.getByRole('button',{name:'結果に戻る'}).click();
